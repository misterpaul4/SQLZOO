...
SELECT DISTINCT player FROM goal JOIN game ON matchid = id WHERE (teamid!='GER' AND (team2='GER' OR team1='GER'));
...
SELECT matchid,mdate, COUNT(mdate) FROM game JOIN goal ON id = matchid WHERE (team1='POL' OR team2='POL') GROUP BY mdate ORDER BY matchid;

--12. For every match where 'GER' scored, show matchid, match date and the number of goals scored by 'GER'--
SELECT matchid, mdate, COUNT(teamid) FROM game JOIN goal ON id=matchid WHERE teamid='ger' GROUP BY id ORDER BY id;
...
FROM game LEFT JOIN goal ON matchid = id GROUP BY matchid, team1, team2 ORDER BY mdate, matchid, team1, team2;